...
int main(int argc, char *argv[]){
 ...
 /* create the child pipe*/
  ...
  if (pid > 0) {  /* parent process */
    ...
    /* write to the pipe */
    write(fdP[WRITE_END], finstr, strlen(finstr)+1); 

    /* close the write end of the pipe */
    close(fdP[WRITE_END]);

    while(block_number != var1){
      /* close the unused end of the child pipe */
      close(fdC[WRITE_END]);

      /* read from the child pipe */
      read(fdC[READ_END], read_msg, BUFFER_SIZE);
  ...
  /* child process */
    ...
    //var1 is initalized to -1
    while(var1 != 0){
      
    // printf("CHILD PROCESS WHILE LOOP\n");

    /* close the unused end of the pipe */
    close(fdP[WRITE_END]);

    /* read from the pipe */
    read(fdP[READ_END], read_msg, BUFFER_SIZE);
    ...
    /*  write to the pipe */
    write(fdC[WRITE_END], write_msg, strlen(write_msg)+1); 

    /* close the write end of the pipe */
    close(fdC[WRITE_END]);

    ++child_block; //increment child_block count
    } //end if child_block check
  } //end while loop
