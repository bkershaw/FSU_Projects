...
namespace fsu
{
  ...
  HashTableIterator<K,D,H> HashTable<K,D,H>::Insert (const K& k, const D& d)
  {
    ...
    b = hashObject_(k) % numBuckets_;
    
    typename BucketType::Iterator i2 = (bucketVector_[b]).Includes(e);

    if(i2 != bucketVector_[b].End())
    {
      (*i2).data_ = d;
    }
    else
    {
      bucketVector_[b].Insert(e);
    }
  ...
  bool HashTable<K,D,H>::Remove (const K& k)
  {
    ...
    if( i2 != bucketVector_[b].End() )
    {
      bucketVector_[b].Remove(i2);
      return 1; //remove succesful
    }
  ...
  bool HashTable<K,D,H>::Retrieve (const K& k, D& d) const
  {
    ...
    typename BucketType::ConstIterator i2 = (bucketVector_[b]).Includes(e);
  ...
  HashTableIterator <K,D,H>& HashTableIterator<K,D,H>::operator ++ ()
  {
    ...
    else //it's at the end of the bucket
    {
          do
          {
            ++bucketNum_;
            ...
            {
              *this = tablePtr_->End();
              return *this; //return end iterator
            }
